...
def get_augmented_examples_mask(examples):
    mel_spec = []
    ...
    mel_spec = mel_spec.swapaxes(1, 0)
    augmented_mel = augment_mask(mel_spec)
    augmented_mel = augmented_mel.swapaxes(1, 0)

    augmented_examples = []
    examples_batch = []
    for index, frame in enumerate(augmented_mel):
        if int(index + 1) % 96 == 0:
            examples_batch.append(frame)
            augmented_examples.append(examples_batch)
            examples_batch = []
        else:
            examples_batch.append(frame)
...
def get_augmented_examples_warp(examples):
    mel_spec = []
    for example in examples:
        for frame in example:
            mel_spec.append(frame)
    ...
    mel_spec = np.array(mel_spec)
    mel_spec = mel_spec.swapaxes(1, 0)
    augmented_mel = augment_warp(mel_spec)
    augmented_mel = augmented_mel.swapaxes(1, 0)
